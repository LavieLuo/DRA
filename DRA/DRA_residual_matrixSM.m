function [uu, rr, time] = DRA_residual_matrixSM(TrainFea,TrainG,ValidFea,ValidG,type)
%type: only-right model with type=0, all-in model with type = 1.
% Using Sherman¨CMorrison¨CWoodbury formula.
lambda = 0.01;
...
if type==0
    ...
    for i = unique(ValidG)
        ...
        Lsuc = LY(:,m) - LUC(:,n2);
        clear LUC
        Lgama = LSuC'*Lsuc/lambda - LSuC'*((eye(size(LSuC,1))+LSuC*LSuC'/lambda)\(LSuC*(LSuC'*Lsuc)))/lambda^2;
